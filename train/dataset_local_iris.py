...
class Dataset_train(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        local_iris = local_outer - local_inner
        augmented = self.transforms(image=img, mask=local_iris)
        img, local_iris = augmented['image'], augmented['mask']
        img = torch.from_numpy(img).float().unsqueeze(0)
        local_iris = torch.from_numpy(local_iris).float().unsqueeze(0)
        return img, local_iris
...
if __name__ == '__main__':
    ...
    for i, (img, local_iris) in enumerate(train_dataloader):
        img = img[0, 0, :, :].numpy() * 255
        local_iris = local_iris[0, 0, :, :].numpy() * 255
        plt.subplot(121)
        plt.imshow(img, cmap='gray')
        plt.subplot(122)
        plt.imshow(local_iris, cmap='gray')
        plt.show()