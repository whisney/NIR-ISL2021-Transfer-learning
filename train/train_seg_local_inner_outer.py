import os
from dataset_seg_local_inner_outer import Dataset_train, Dataset_val
...
save_dir = os.path.join('trained_models/{}/Seg_Local_inner_outer/{}_{}'.format(description, args.name, args.net), save_name)
...
for epoch in range(args.epoch):
    ...
    for i, (inputs, segs, local_inners, local_outers) in enumerate(train_dataloader):
        inputs, segs, local_inners, local_outers = inputs.float().cuda(), segs.float().cuda(), \
                                                   local_inners.float().cuda(), local_outers.float().cuda()
        optimizer.zero_grad()
        result_seg, result_local_inner, result_local_outer = net(inputs)
        result_seg = torch.sigmoid(result_seg)
        result_local_inner = torch.sigmoid(result_local_inner)
        result_local_outer = torch.sigmoid(result_local_outer)
        segloss = criterion(result_seg, segs)
        innerloss = criterion(result_local_inner, local_inners)
        outerloss = criterion(result_local_outer, local_outers)
        total_loss = segloss + 0.5 * innerloss + 0.5 * outerloss
        total_loss.backward()
        optimizer.step()

        pred_seg = result_seg.cpu().float()
        pred_seg[pred_seg <= 0.5] = 0
        pred_seg[pred_seg > 0.5] = 1
        seg_acc = ((pred_seg == segs.cpu()).sum().float() / pred_seg.numel()).detach().numpy()
        ...
        epoch_train_segloss.append(segloss.item())
        epoch_train_segacc.append(seg_acc)
        ...
        print('[%d/%d, %5d/%d] train_total_loss: %.3f seg_acc: %.3f inner_dice: %.3f outer_dice: %.3f' %
              (epoch + 1, args.epoch, i + 1, math.ceil(train_data_len / args.bs), total_loss.item(), seg_acc,
    ...
    with torch.no_grad():
        for i, (inputs, segs, local_inners, local_outers) in enumerate(val_dataloader):
            inputs, segs, local_inners, local_outers = inputs.float().cuda(), segs.float().cuda(), \
                                                       local_inners.float().cuda(), local_outers.float().cuda()
            result_seg, result_local_inner, result_local_outer = net(inputs)
            result_seg = torch.sigmoid(result_seg)
            ...
            epoch_val_segloss.append(segloss.item())
            epoch_val_segacc.append(seg_acc)
            ...
            if i in [0, 1, 2, 3] and epoch % (args.epoch // 10) == 0:
                PREDICTION_SEG.append(pred_seg[0:1, :, :, :])
    ...
    epoch_train_segloss = np.mean(epoch_train_segloss)
    epoch_train_segacc = np.mean(epoch_train_segacc)
    ...
    epoch_val_segloss = np.mean(epoch_val_segloss)
    epoch_val_segacc = np.mean(epoch_val_segacc)
    ...
    print('[%d/%d] train_totalloss: %.3f  val_totalloss: %.3f  val_segacc: %.3f val_innerdice: %.3f val_outerdice: %.3f'
          % (epoch + 1, args.epoch, epoch_train_totalloss, epoch_val_totalloss, epoch_val_segacc, epoch_val_innerdice,
             epoch_val_outerdice))

    train_writer.add_scalar('lr', lr, epoch)
    train_writer.add_scalar('total_loss', epoch_train_totalloss, epoch)
    train_writer.add_scalar('seg_loss', epoch_train_segloss, epoch)
    ...
    train_writer.add_scalar('seg_acc', epoch_train_segacc, epoch)
    ...
    if epoch % (args.epoch // 10) == 0:
        PREDICTION_SEG = torch.cat(PREDICTION_SEG, dim=0)
        ...
        PREDICTION_SEG = make_grid(PREDICTION_SEG, 2, normalize=True)
        ...
        val_writer.add_image('PREDICTION_SEG', PREDICTION_SEG, epoch)
    ...
    if epoch_val_segacc > best_seg_acc:
        best_seg_acc = epoch_val_segacc
        torch.save(net.state_dict(),
                   os.path.join(save_dir, 'best_seg_acc.pth'))
