...
class Dataset_train(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        seg_local = np.concatenate((seg[:, :, np.newaxis], local_inner[:, :, np.newaxis],
                                    local_outer[:, :, np.newaxis]), axis=2)
        augmented = self.transforms(image=img, mask=seg_local)
        img, seg_local = augmented['image'], augmented['mask']
        seg = seg_local[:, :, 0]
        local_inner = seg_local[:, :, 1]
        local_outer = seg_local[:, :, 2]
        ...
        return img, seg, local_inner, local_outer
...
if __name__ == '__main__':
    train_data = Dataset_val(data_root=r'I:\public_data\NIR-ISL2021',
                               ...
                               size=(1088, 640), fold=0, resize=True)
    train_dataloader = DataLoader(dataset=train_data, batch_size=1, shuffle=True)
    for i, (img, seg, local_inner, local_outer) in enumerate(train_dataloader):
